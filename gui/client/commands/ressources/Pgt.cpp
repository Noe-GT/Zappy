...
void zappyGUI::Pgt::receive(std::string command, zappyGUI::GUI &gui)
{
    ...
    if (gui.getGame()->getMap()[playerPos.first][playerPos.second].getRessources()[ressourceID].second < 0)
        gui.getGame()->getMap()[playerPos.first][playerPos.second].getRessources()[ressourceID].second = 0;
