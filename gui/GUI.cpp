...
zappyGUI::GUI::GUI(int port, std::string hostname): _window(), _client(port, hostname)
{
    ...
    std::shared_ptr<UIBlocks::List> list = std::make_shared<UIBlocks::List>(options, std::pair<float, float>(1200, 200), std::pair<float, float>(100, 200));
    ...
    selector.get()->open();
    std::shared_ptr<UIBlocks::Timer> timer = std::make_shared<UIBlocks::Timer>(std::pair<float, float>(20, 20), 50);
    timer.get()->start();
